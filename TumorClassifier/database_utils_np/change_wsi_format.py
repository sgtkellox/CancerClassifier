...
def changeFile(slidePath, outPath):
    ...
    # Set the final size
    ds = tiff_drv.Create(
        os.path.join(outPath,filename.split('.')[0] + '.tif'),
...
if __name__ == '__main__':
    ...
    argParser.add_argument("-s", "--slides", help="The path to the folder containing the slides to be altered")
    #argParser.add_argument("-r", "--recursive", default=True, help="Recursivly check subfolders")
    argParser.add_argument("-f", "--format", help="The file extension of the proprietary file format to be altered")
    argParser.add_argument("-o", "--out", help="The path to where the resulting slides go")
    ...
    for path, subdirs, files in os.walk(slidesPath):
        for fileName in files:
            print(fileName)
            if fileName.endswith(fileFormat):

                filePath = os.path.join(slidesPath,fileName)
                    
                changeFile(filePath,outPath)
