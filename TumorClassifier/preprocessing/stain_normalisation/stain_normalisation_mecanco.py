...
def nomalizeStainInFolder(inPath,outPath):
    ...
    for img in imgs:
        filePath = os.path.join(inPath,img)
        fileName = img.split(".")[0]
        img=cv2.imread(filePath, 1)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

        Io = 200 # Transmitted light intensity, Normalizing factor for image intensities
        ...
        plt.imsave(os.path.join(outPath,"result","macenko","normalize",fileName+"_norm.jpg"), Inorm)
        plt.imsave(os.path.join(outPath,"result","macenko","hematoxylin",fileName+"_h.jpg"), H)
        plt.imsave(os.path.join(outPath,"result","macenko","eosin",fileName+"_e.jpg"), E)


if __name__ == '__main__':
    inPath = r"C:\Users\felix\Desktop\neuro\stainTestIn"
    outPath = r"C:\Users\felix\Desktop\neuro\stainTest\origMacenko\run2"
    nomalizeStainInFolder(inPath,outPath)
